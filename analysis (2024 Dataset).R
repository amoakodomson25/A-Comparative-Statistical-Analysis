...
# Load the dataset
data <- read_excel("2017-2018 data set.xlsx", sheet = "2024")
...
ggplot(combined_plot_data, aes(x = Gender, y = Value, fill = Locality)) +
  ...
  geom_text(aes(label = round(Value, 1)), 
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Urban vs Rural by Gender (2017–2018 Dataset)",
    x = "Gender",
    y = "Percentage"
  ) +
  theme_minimal() +
...
ggplot(compare_data, aes(x = Locality, y = Value, fill = Category)) +
  ...
  geom_text(aes(label = round(Value, 1)),
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Comparison of Never Attended vs Formal Education by Locality (2017–2018 Dataset)",
    x = "Locality",
    y = "Percentage"
  ) +
  theme_minimal() +
...
ggplot(male_data, aes(x = Education.Level, y = Value, fill = Locality)) +
  ...
  geom_text(aes(label = round(Value, 1)), 
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Education Levels of Males by Locality (2017–2018 Dataset)",
    x = "Education Level",
    y = "Percentage"
  ) +
  theme_minimal()
...
ggplot(female_data, aes(x = Education.Level, y = Value, fill = Locality)) +
  ...
  geom_text(aes(label = round(Value, 1)), 
            ...
            vjust = -0.5, size = 3) +
  labs(
    title = "Education Levels of Females by Locality (2017–2018 Dataset)",
    x = "Education Level",
    y = "Percentage"
  ) +
  theme_minimal()
