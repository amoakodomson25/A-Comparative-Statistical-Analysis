...
tryCatch({
  ...
  # Calculate standard deviation by education level (1 decimal places)
  edu_sd <- clean_data %>%
    group_by(Education) %>%
    summarise(
      Male_SD = format(round(sd(Male), 1), nsmall = 1),
      Female_SD =format(round(sd(Female), 1), nsmall = 1),
      SD_Ratio = round(sd(Male)/sd(Female), 2),  # Male-to-female variability ratio
      .groups = 'drop'
    )
  
  # Print formatted results
  cat("Standard Deviation of Participation by Education Level:\n")
  print(edu_sd, n = Inf)
